...
function Feature({ title, text, wide = false, children }: { title: string; text: string; wide?: boolean; children: ReactNode }) {
  return (
    <li className={cn("flex flex-col gap-6 rounded-3xl bg-desk p-6 sm:p-8", wide && "md:col-span-2")}>
    ...
    <section id="fonctionnalites" className={cn("bg-paper py-24 lg:py-35", sectionX)} aria-labelledby="features-title">
      <div className="mx-auto flex max-w-[1312px] flex-col gap-16">
        ...
        <ul className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          >
            <div className="flex gap-3">
              {sourcedFacts.map(([fact, source], i) => (
                <div key={fact} className={cn("w-[220px] shrink-0 flex-col gap-2 rounded-xl bg-desk p-4", i === 2 ? "hidden xl:flex" : "flex")}>
                  <span className="text-sm leading-[1.4]">
                    <span className="hl">{fact}</span>
                  </span>
          ...
          <Feature title="Votre ton, votre langue" text="Les messages suivent votre façon de parler et l’appel à l’action que vous choisissez.">
            <div className="flex flex-col items-center gap-3.5">
              <div className="flex max-w-[280px] flex-wrap justify-center gap-2 text-sm font-semibold">
                <span className="flex h-9 items-center rounded-full bg-ink px-3.5 text-paper">Professionnel</span>
                <span className="flex h-9 items-center rounded-full border border-line px-3.5">Direct</span>
                <span className="flex h-9 items-center rounded-full border border-line px-3.5">Chaleureux</span>
                <span className="flex h-9 items-center rounded-full border border-line px-3.5">Consultatif</span>
              </div>
              <span className="font-mono text-xs text-graphite">Français · English</span>
            </div>
          </Feature>

          <Feature title="Un rythme d’envoi prudent" text="Les envois sont espacés et plafonnés chaque jour, pour protéger votre adresse.">
            <div className="flex flex-col items-center gap-4">
              <div className="flex items-center">
                {[0, 1, 2, 3, 4].map((i) => (
                  <div key={i} className="flex items-center">
                    {i > 0 && <span className={cn("h-px w-6.5", i <= 2 ? "bg-ink" : "bg-rule")} />}
                    <span className={cn("size-3.5 rounded-full", i <= 2 ? "bg-ink" : "border-[1.5px] border-ink")} />
                  </div>
                ))}
              </div>
          ...
          <Feature title="Liste d’exclusion" text="Une adresse ou un domaine exclu ne peut plus être ciblé par vos campagnes.">
            <div className="flex w-full flex-col gap-2.5 text-sm">
              {exclusions.map(([address, label]) => (
                <div key={address} className="flex items-center justify-between gap-2.5">
                  <span className="truncate text-graphite line-through decoration-danger">{address}</span>
          ...
          <Feature title="Suivi des réponses" text="Envoyé, répondu, désinscrit : le statut de chaque prospect se met à jour.">
            <div className="flex w-full max-w-[280px] items-start gap-3">
              ...
              <div className="flex min-w-0 flex-col gap-1">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <span className="rounded-full bg-success-soft px-2 py-0.5 text-[11px] font-semibold text-success">Réponse</span>
                </div>
          ...
          >
            <div className="flex w-full flex-col gap-4 px-2 sm:flex-row sm:items-center sm:gap-12 sm:px-6">
              <p className="flex items-baseline gap-2.5">
                <span className="text-[clamp(56px,5vw,88px)] leading-none font-semibold tracking-[-0.05em] tabular-nums">112</span>
                <span className="text-lg text-graphite">/ 150 crédits</span>
              </p>
              <div className="flex flex-1 flex-col gap-3">
                <div className="h-2 overflow-hidden rounded bg-desk">
                  <div className="h-full w-3/4 rounded bg-ink" />
                </div>
          ...
          <Feature title="Gmail et Outlook" text="Connexion sécurisée en deux clics, sans partager votre mot de passe.">
