...
import { CheckItem, delay, paperShadow, SectionHeading, sectionX } from "./common";
...
function Step({
  ...
  return (
    ...
    >
      <div data-reveal className={cn("flex flex-col gap-5.5", reverse && "lg:order-2")}>
  ...
  return (
    <div className={cn("flex w-full max-w-[520px] flex-col gap-5 rounded-2xl bg-paper p-6 sm:p-8", paperShadow)}>
      ...
      <p className="rounded-xl border-[1.5px] border-ink px-4.5 py-4 text-[19px] leading-[1.4]">
        <span data-type data-type-delay="600">
      ...
      <p data-reveal="fade" style={delay(30)} className="flex items-center gap-2 font-mono text-[11px] tracking-[0.04em] text-graphite uppercase">
      ...
      <dl className="grid gap-2.5 sm:grid-cols-2">
        {fields.map(([label, value], i) => (
          <div
            ...
            data-reveal
            style={delay(32 + i * 2)}
            className="flex items-center justify-between rounded-[10px] border border-rule px-3.5 py-3"
          >
      ...
      <div data-reveal="fade" style={delay(42)} className="flex flex-wrap items-center justify-between gap-3">
  ...
  return (
    <>
      <svg className="absolute inset-0 size-full" viewBox="0 0 740 480" preserveAspectRatio="xMidYMid slice" fill="none" aria-hidden="true">
        ...
        <g fill="#b9b8b3">
          {otherBusinesses.map(([cx, cy], i) => (
            <circle key={`${cx}-${cy}`} data-pop style={delay(i, 800)} cx={cx} cy={cy} r="5" />
          ))}
        </g>
        <g fill="#fdf156" stroke="#12110f" strokeWidth="1.5">
          {matches.map(([x, y], i) => (
            <rect key={`${x}-${y}`} data-pop style={delay(i * 2, 1600)} x={x} y={y} width="14" height="14" />
          ))}
        </g>
      </svg>
      <dl
        data-reveal
        style={delay(8)}
        className={cn("absolute top-6 left-6 flex min-w-[230px] flex-col gap-2.5 rounded-2xl bg-paper px-5 py-4.5 text-sm", paperShadow)}
      >
        ...
        {results.map(([label, value]) => (
          <div key={label} className="flex justify-between gap-7">
            <dt>{label}</dt>
            <dd className="font-mono font-medium" data-count={value}>
              {value}
            </dd>
          </div>
        ))}
      ...
      <div
        data-reveal="fade"
        style={delay(24)}
        className="absolute bottom-6 left-6 flex flex-wrap gap-x-4.5 gap-y-1 rounded-[10px] bg-paper px-3.5 py-2.5 text-[13px]"
      >
      ...
      <span data-reveal="fade" style={delay(24)} className="absolute right-7 bottom-7.5 hidden font-mono text-xs tracking-[0.06em] sm:block">
        RAYON 15 KM
      </span>
    </>
  );
...
function SiteToEmailVisual() {
  return (
    <div className="flex flex-col items-center gap-5 sm:flex-row">
      <div data-reveal style={delay(3)} className={cn("w-[280px] overflow-hidden rounded-xl bg-paper xl:w-[300px]", paperShadow)}>
        ...
        <div className="flex flex-col gap-3 p-5">
          ...
          <span className="text-[13px]">
            <span data-sweep style={delay(0, 1100)} className="hl-solid px-1">
              Menu du midi : voir Instagram
            </span>
          </span>
          <span className="h-2 w-[90%] rounded bg-rule-soft" />
          <span className="text-[13px]">
            <span data-sweep style={delay(1, 1100)} className="hl-solid px-1">
              Réservations au 04 •• •• •• ••
            </span>
          </span>
      ...
      <div data-reveal style={delay(26)} className={cn("flex w-[280px] flex-col gap-3 bg-paper p-5.5 xl:w-[300px]", paperShadow)}>
        ...
        <p className="font-serif text-sm leading-[1.6]">
          Bonjour, j’ai vu que{" "}
          <span data-sweep style={delay(0, 3100)} className="hl">
            votre menu du midi n’est publié que sur Instagram
          </span>{" "}
          et que{" "}
          <span data-sweep style={delay(1, 3100)} className="hl">
  ...
  return (
    <div className="flex w-full flex-col gap-4 sm:block sm:min-h-[416px]">
      <div
        data-reveal
        style={delay(3)}
        className={cn("flex w-full flex-col rounded-2xl bg-paper px-5.5 py-5 sm:absolute sm:top-11 sm:left-10 sm:w-[380px]", paperShadow)}
      >
        ...
        {rows.map((row, i) => (
          <div
            ...
            data-reveal="fade"
            style={delay(7 + i * 2)}
            className="flex h-11.5 items-center justify-between border-t border-rule-soft text-[15px]"
          >
            <span className={cn(row.state === "rejected" && "text-graphite line-through decoration-danger")}>{row.name}</span>
            {row.state === "approved" && (
              <span data-pop style={delay(i, 1500)} className="flex items-center gap-1.5 text-[13px] font-semibold text-success">
      ...
      <div
        data-reveal
        style={delay(22)}
        className="flex w-full flex-col gap-5.5 rounded-2xl bg-paper p-7 shadow-[0_30px_60px_-20px_rgba(18,17,15,0.35)] sm:absolute sm:right-10 sm:bottom-11 sm:w-[360px]"
      >
